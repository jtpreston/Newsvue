...
router.get('/newsinsights', (req, res) => {
  const { start = 0, end = 9999999999999, limit = 100, topic = 'smart phones' } = req.query;
  entitiesDB.aggregateEntities(parseInt(start), parseInt(end), parseInt(limit), topic).then(results => {
...
/** Fetch articles based on custom topic and store in database */
router.get('/topic', (req, res) => {
	const { topic = 'smart phones' } = req.query;
	//need to make call like app.js does to newsScraper
	entitiesDB.getMinAndMaxDates().then(minAndMax => {
    const start = minAndMax.max ? minAndMax.max/1000 : null;
    return newsScraper(start + 1, topic);
  }).then(entities =>
    entitiesDB.uploadArticlesFromDocs(entities, topic, function() {
    	res.json("Success");
    })
  ).catch(e => {
    ...
    console.error(e);
    res.json(e);
    //add this in later when everything else works
//    if (++failureCount > 5) {
